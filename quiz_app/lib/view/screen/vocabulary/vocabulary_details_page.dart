...
class VocabularyDetailsPage extends StatelessWidget {
  ...
  const VocabularyDetailsPage({Key? key, required this.endPoint, required this.subjectName}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<VocabularyModel?>(
        future: getPdfs(),
  ...
  Future<VocabularyModel?> getPdfs() async {
    var response = await tokenBasedGet(url: ApiUrl.apiUrl + endPoint);
    if (response == null) {
      return null;
    } else {
      return vocabularyModelFromJson(response.body);
    }
  }
}
