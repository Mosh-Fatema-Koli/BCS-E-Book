...
class QuizDetailsPage extends StatelessWidget {
  ...
  const QuizDetailsPage({Key? key, required this.endPoint, required this.subjectName}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<EbookModel?>(
        future: getPdfs(),
        builder: (context, AsyncSnapshot<EbookModel?> snapshot) {
  ...
  Future<EbookModel?> getPdfs() async {
    var response = await tokenBasedGet(url: ApiUrl.apiUrl + endPoint);
    if (response == null) {
      return null;
    } else {
      return ebookModelFromJson(response.body);
    }
  }
}
